...
public class OrderService {
    ...
    //查看订单 By OpenId
    public List<OmsOrder> getOrderListByOpenId(String openId, Integer paymentStatus) {
        OmsOrderExample example = new OmsOrderExample();
        if (paymentStatus == null) {
            example.createCriteria().andOpenIdEqualTo(openId);
        } else {
            example.createCriteria().andOpenIdEqualTo(openId).andStatusEqualTo(paymentStatus);
        }
    ...
    public int applyRefund(long orderId){
        ...
        if (order.getStatus() != PaymentStatus.PAID.getStatus()){
            logger.error("只有支付完成的订单可以申请退款，订单号"+orderId+"订单状态"+order.getStatus());
            return 0;
        }
    ...
    public int cancelRefund(long orderId){
        ...
        if (order.getStatus() != PaymentStatus.APPLY_REFUND.getStatus()){
            logger.error("未申请退款，无法取消退款，订单号"+orderId+"当前订单状态"+order.getStatus());
            return 0;
        }
        order.setStatus(PaymentStatus.REFUND.getStatus());
        return orderMapper.updateByPrimaryKey(order);
